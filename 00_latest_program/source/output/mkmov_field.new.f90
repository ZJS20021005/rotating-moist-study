...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      use local_arrays, only: q3,q2,q1,dens,dsal,qvap
      use mpi_param
      use mpih
      use hdf5
      use param
!      use stat_arrays
      ...
      real, allocatable, dimension(:,:,:) :: rh_mea
      ...
      real :: strn, omeg
      real :: tloc, qsatloc
      real, parameter :: qsat_eps = 1.0d-20
...
!      integer(HID_T) :: dset_qtens
      ...
      call update_both_ghosts(n1,n2,qvap,kstart,kend)
      call update_both_ghosts(n1,n2,qs,kstart,kend)
     ! call update_both_ghosts(n1,n2,vx_me,kstart,kend)
      ...
      allocate(rh_mea(1:n1m,1:n2m,kstart:kend))
      ...
      do kc=kstart,kend
        do jc=1,n2m
          do ic=1,n1m
           ...
           ! Post-processing RH based on local dsal and z.`r`n           tloc = (1.0d0-betaqs)*dsal(ic,jc,kc) - betaqs*zc(kc)`r`n           qsatloc = exp(alphaqs*tloc)`r`n           if (qsatloc > qsat_eps) then`r`n             rh_mea(ic,jc,kc) = qvap(ic,jc,kc) / qsatloc`r`n           else`r`n             rh_mea(ic,jc,kc) = 0.0d0`r`n           endif
          end do
        end do
      end do
      ...
      call h5dcreate_f(file_temp, 'RH_me', H5T_NATIVE_DOUBLE,filespace, dset_RH, hdf_error)
     ...
     &   xfer_prp = slab_plist)
      ...
      call h5dwrite_f(dset_RH, H5T_NATIVE_DOUBLE,&
     &   rh_mea(1:n1m,1:n2m,kstart:kend), dims, &
     &   hdf_error, file_space_id = slabspace, mem_space_id = memspace, &
     &   xfer_prp = slab_plist)
     ...
     & n3m,n2m,n1m
      ...
      write(45,'("<Attribute Name=""RH_me""&
     & AttributeType=""Scalar"" Center=""Node"">")')
      write(45,'("<DataItem Dimensions=""",i4," ",i4," ",i4,"""&
     & NumberType=""Float"" Precision=""4"" Format=""HDF"">")')&
     & n3m,n2m,n1m
      write(45,'("field",i5.5,".h5:/RH_me")') itime
      write(45,'("</DataItem>")')
      write(45,'("</Attribute>")')
      ...
      if(allocated(rh_mea)) deallocate(rh_mea)
