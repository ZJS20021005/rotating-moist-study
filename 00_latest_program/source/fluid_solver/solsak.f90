      ...
      do jc=jstartr,jendr
        do ic=1,n1mr
          do kc=1,n3mr
            ...
            kp=kpvr(kc)
            if(kc.eq.1) then        
               K_p = (kpat(kp,ic,jc)*(zcr(kp)-zmr(kc)) + kpat(kc,ic,jc)*(zmr(kp)-zcr(kp)))/(zmr(kp)-zmr(kc))
               K_m = (kpat(kc,ic,jc) + kpat(km,ic,jc))*0.5d0
            elseif( (kc.ge.2) .and. (kc.le.n3mr-1) ) then
               K_p = (kpat(kp,ic,jc)*(zcr(kp)-zmr(kc)) + kpat(kc,ic,jc)*(zmr(kp)-zcr(kp)))/(zmr(kp)-zmr(kc))
               K_m = (kpat(kc,ic,jc)*(zcr(kc)-zmr(km)) + kpat(km,ic,jc)*(zmr(kc)-zcr(kc)))/(zmr(kc)-zmr(km))
            elseif(kc.eq.n3mr) then
               K_p = (kpat(kp,ic,jc) + kpat(kc,ic,jc))*0.5d0
               K_m = (kpat(kc,ic,jc)*(zcr(kc)-zmr(km)) + kpat(km,ic,jc)*(zmr(kc)-zcr(kc)))/(zmr(kc)-zmr(km))
            endif
            K_c = (K_p+K_m)
