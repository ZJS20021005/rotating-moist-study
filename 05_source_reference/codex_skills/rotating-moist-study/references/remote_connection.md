...
This platform uses Slurm partition `xhacnormalc`. The continuation cases use
one node and 64 MPI ranks. Prepare and validate jobs with
`04_platform_scripts/submit_all.sh`; do not add `--submit` without the user's
explicit approval.
